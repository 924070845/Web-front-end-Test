...
function checkBirthday() {
    var reg = /^[\d]{4}[-\ ][\d]{1,2}[-\ ][\d]{1,2}$/;/* 这个比较简单，不说了 */
    var id = "PBirthday";
    return commonInfo(reg, id);
}
...
function checkSames() {
    var reg = /^\d{6}$/ig;/* 必须是数字，位数为6位 */
    var inputText = document.getElementById("aPPassword");
    var inputValue = inputText.value;
    var inputAlt = inputText.alt;
    var inputSpan = document.getElementById("aPPasswordSpan");
    var first = document.getElementById("PPassword").value;
    if (inputValue == null || inputValue.length === 0) {
        inputSpan.innerHTML = inputAlt + "不能为空！";
        inputSpan.style.color = "red";
        return false;
